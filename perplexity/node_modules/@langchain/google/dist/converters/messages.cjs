...
*/
function convertStandardContentMessageToGeminiContent(message) {
	...
	if (_langchain_core_messages.HumanMessage.isInstance(message)) role = "user";
	...
	else if (_langchain_core_messages.ToolMessage.isInstance(message)) role = "function";
		...
		else if (msgRole === "function" || msgRole === "tool") role = "function";
		else role = "user";
	} else role = "user";
	const parts = [];
...
function convertLegacyContentMessageToGeminiContent(message, messages) {
	...
	*/
	...
	const role = require_misc.iife(() => {
		...
		else if (_langchain_core_messages.ToolMessage.isInstance(message)) return "function";
		else if (_langchain_core_messages.ChatMessage.isInstance(message)) {
			const msgRole = message.role.toLowerCase();
			if (msgRole === "user" || msgRole === "human") return "user";
			else if (msgRole === "assistant" || msgRole === "ai" || msgRole === "model") return "model";
			else if (msgRole === "function" || msgRole === "tool") return "function";
			else return "user";
		} else return "user";
	});
	let parts = [];
	...
	if (role === "function") parts = parts.filter((part) => "functionResponse" in part);
...
const convertGeminiPartsToToolCalls = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if ("functionCall" in part && part.functionCall) {
			...
			toolCalls.push({
				type: "tool_call",
				id: functionCallPart.functionCall.id ?? (0, uuid.v4)().replace(/-/g, ""),
