...
function adjustObjectType(obj) {
	...
	if (len === 2 && nullIndex >= 0) {
		...
		obj.nullable = true;
	} else if (len === 1 && nullIndex === 0) throw new InvalidInputError("Gemini does not support null-only types in function schemas. Provide a non-null type alongside nullable.");
	else if (len === 1) obj.type = obj?.type[0];
	else if (len === 0) throw new InvalidInputError("Gemini does not support empty type arrays in function schemas. Provide at least one type.");
	else throw new InvalidInputError("Gemini does not support union types in function schemas. Use a single type instead.");
	return obj;
}
/**
...
//#endregion
export { convertToolChoiceToGeminiConfig, convertToolsToGeminiTools, schemaToGeminiParameters };
