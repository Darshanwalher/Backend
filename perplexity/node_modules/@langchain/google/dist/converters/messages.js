...
import { AIMessage, ChatMessage, HumanMessage, SystemMessage, ToolMessage, convertToProviderContentBlock, isDataContentBlock, parseBase64DataUrl } from "@langchain/core/messages";
import { v4 } from "uuid";
	...
	fromStandardImageBlock(block) {
		if (isDataContentBlock(block)) {
			if (block.source_type === "base64") {
				if (!block.mime_type) throw new InvalidInputError("mime_type is required for base64 image blocks");
		...
		throw new InvalidInputError(`Image content blocks with source_type "${block.source_type}" are not supported for ChatGoogle. Supported source types are: "base64", "url", "id".`);
	},
	fromStandardAudioBlock(block) {
		if (isDataContentBlock(block)) {
			if (block.source_type === "base64") {
				if (!block.mime_type) throw new InvalidInputError("mime_type is required for base64 audio blocks");
				const dataStr = typeof block.data === "string" ? block.data : typeof block.data === "object" && block.data !== null ? btoa(Array.from(block.data).map((byte) => String.fromCharCode(byte)).join("")) : String(block.data);
				return { inlineData: {
					mimeType: block.mime_type,
					data: dataStr
		...
		throw new InvalidInputError(`Audio content blocks with source_type "${block.source_type}" are not supported for ChatGoogle. Supported source types are: "base64", "url", "id".`);
	},
	fromStandardFileBlock(block) {
		if (isDataContentBlock(block)) {
			if (block.source_type === "base64") {
				if (!block.mime_type) throw new InvalidInputError("mime_type is required for base64 file blocks");
				const dataStr = typeof block.data === "string" ? block.data : typeof block.data === "object" && block.data !== null ? btoa(Array.from(block.data).map((byte) => String.fromCharCode(byte)).join("")) : String(block.data);
				return { inlineData: {
					mimeType: block.mime_type,
					data: dataStr
		...
		throw new InvalidInputError(`File content blocks with source_type "${block.source_type}" are not supported for ChatGoogle. Supported source types are: "base64", "url", "id", "text".`);
	}
};
...
function convertStandardContentMessageToGeminiContent(message) {
	if (SystemMessage.isInstance(message)) return null;
	let role;
	if (HumanMessage.isInstance(message)) role = "user";
	else if (AIMessage.isInstance(message)) role = "model";
	else if (ToolMessage.isInstance(message)) role = "function";
...
function convertLegacyContentMessageToGeminiContent(message, messages) {
	...
	*/
	...
	function messageContentImageUrlData(content) {
		...
		if (!url) throw new InvalidInputError("Missing image URL in image_url content block.");
	...
	function messageContentMediaData(content) {
		...
		throw new InvalidInputError(`Invalid media content: ${JSON.stringify(content, null, 1)}. Expected either { mimeType, data } for inline data or { mimeType, fileUri } for file references.`);
	}
	...
	const role = iife(() => {
		...
		else if (AIMessage.isInstance(message)) return "model";
		else if (ToolMessage.isInstance(message)) return "function";
		else if (ChatMessage.isInstance(message)) {
			const msgRole = message.role.toLowerCase();
	...
	if (typeof message.content === "string") {
		...
		else if (isDataContentBlock(item)) parts.push(convertToProviderContentBlock(item, geminiContentBlockConverter));
	...
	if (ToolMessage.isInstance(message) && message.tool_call_id) {
		const responseContent = typeof message.content === "string" ? message.content : JSON.stringify(message.content);
		const aiMsg = messages.filter(AIMessage.isInstance).find((msg) => msg.tool_calls?.find((tc) => tc.id === message.tool_call_id));
...
const convertMessagesToGeminiSystemInstruction = (messages) => {
	const systemParts = [];
	for (const message of messages) if (SystemMessage.isInstance(message)) {
...
const convertGeminiPartsToToolCalls = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if ("functionCall" in part && part.functionCall) {
			...
			toolCalls.push({
				type: "tool_call",
				id: functionCallPart.functionCall.id ?? v4().replace(/-/g, ""),
...
//#endregion
export { convertAIMessageToText, convertGeminiCandidateToAIMessage, convertGeminiGenerateContentResponseToUsageMetadata, convertGeminiPartsToToolCalls, convertMessagesToGeminiContents, convertMessagesToGeminiSystemInstruction };
