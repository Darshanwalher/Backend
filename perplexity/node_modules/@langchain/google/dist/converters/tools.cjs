...
function adjustObjectType(obj) {
	...
	if (len === 2 && nullIndex >= 0) {
		...
		obj.nullable = true;
	} else if (len === 1 && nullIndex === 0) throw new require_errors.InvalidInputError("Gemini does not support null-only types in function schemas. Provide a non-null type alongside nullable.");
	else if (len === 1) obj.type = obj?.type[0];
	else if (len === 0) throw new require_errors.InvalidInputError("Gemini does not support empty type arrays in function schemas. Provide at least one type.");
	else throw new require_errors.InvalidInputError("Gemini does not support union types in function schemas. Use a single type instead.");
	return obj;
}
/**
...
*/
const convertToolsToGeminiFunctionDeclarations = (tools) => {
	const functionDeclarations = [];
	for (const tool of tools) {
		if (isGeminiTool(tool) && !("functionDeclarations" in tool)) continue;
		if (isGeminiTool(tool) && "functionDeclarations" in tool && Array.isArray(tool.functionDeclarations)) {
		...
		throw new require_errors.InvalidToolError(tool);
	}
...
*/
const convertToolsToGeminiTools = (tools) => {
	if (tools.length === 0) return [];
	const geminiTools = [];
	const functionDeclarations = [];
	for (const tool of tools) if (isGeminiTool(tool)) if ("functionDeclarations" in tool) geminiTools.push(tool);
	else geminiTools.push(tool);
	else if ("googleSearchRetrieval" in tool) geminiTools.push({ googleSearch: {} });
	else {
...
*/
function convertToolChoiceToGeminiConfig(toolChoice, hasTools) {
	if (!hasTools || toolChoice === void 0) return;
	let mode;
	let allowedFunctionNames;
	let toolChoiceMode;
	let toolChoiceFunction;
	if (typeof toolChoice === "object") {
		toolChoiceMode = toolChoice.mode;
		toolChoiceFunction = toolChoice.function?.name;
	} else toolChoiceMode = toolChoice;
	if (toolChoiceMode === "auto") mode = "AUTO";
	else if (toolChoiceMode === "any" || toolChoiceMode === "required") mode = "ANY";
	else if (toolChoiceMode === "none") mode = "NONE";
	else if (typeof toolChoiceMode === "string") {
		mode = "ANY";
		toolChoiceFunction = toolChoiceMode;
	}
	if (toolChoiceFunction) if (Array.isArray(toolChoiceFunction)) allowedFunctionNames = toolChoiceFunction;
	else allowedFunctionNames = [toolChoiceFunction];
	const functionCallingConfig = {};
	if (allowedFunctionNames?.length) functionCallingConfig.allowedFunctionNames = allowedFunctionNames;
	if (mode) functionCallingConfig.mode = mode;
	return { functionCallingConfig };
}

//#endregion
exports.convertToolChoiceToGeminiConfig = convertToolChoiceToGeminiConfig;
exports.convertToolsToGeminiTools = convertToolsToGeminiTools;
