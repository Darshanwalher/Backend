...
async function getGCPCustomToken(credentials) {
	const privateKey = await getGCPPrivateKey(credentials);
	return await new SignJWT().setIssuer(credentials.client_email).setAudience(credentials.token_uri).setSubject(credentials.client_email).setProtectedHeader({ alg: "RS256" }).setIssuedAt().setExpirationTime("1h").sign(privateKey);
}
/**
...
//#endregion
export { getGCPCredentialsAccessToken, normalizeGCPCredentials };
