...
function normalizeGCPCredentials(credentials) {
	return typeof credentials === "string" ? Object.freeze(JSON.parse(credentials)) : Object.isFrozen(credentials) ? credentials : Object.freeze(credentials);
}
/**
...
function getGCPPrivateKey(credentials) {
	return (0, jose.importPKCS8)(credentials.private_key, "RS256");
}
/**
...
async function getGCPCustomToken(credentials) {
	const privateKey = await getGCPPrivateKey(credentials);
	return await new jose.SignJWT().setIssuer(credentials.client_email).setAudience(credentials.token_uri).setSubject(credentials.client_email).setProtectedHeader({ alg: "RS256" }).setIssuedAt().setExpirationTime("1h").sign(privateKey);
}
/**
...
async function getGCPCredentialsAccessToken(credentials) {
	...
	if (cacheValue) if (cacheValue.created > now - CACHE_MAX_AGE) {
		let token = await cacheValue.promise;
		if (token.expires > now) return token.token;
		else {
	...
	const promise = require_misc.iife(async () => {
		...
		if ("id_token" in data) {
			const claims = (0, jose.decodeJwt)(data.id_token);
			return {
				token: data.id_token,
				expires: claims.exp
			};
		...
		return {
			expires: lastModified ? Math.floor(new Date(lastModified).valueOf() / 1e3) + data.expires_in : now + data.expires_in,
...
//#endregion
exports.getGCPCredentialsAccessToken = getGCPCredentialsAccessToken;
exports.normalizeGCPCredentials = normalizeGCPCredentials;
//# sourceMappingURL=gcp-auth.cjs.map