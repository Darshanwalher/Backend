...
const ns = _langchain_core_errors.ns.sub("google");
/**
...
* ```
*/
var GoogleError = class extends ns.brand(_langchain_core_errors.LangChainError) {
	name = "GoogleError";
};
/**
...
* ```
*/
var ConfigurationError = class extends ns.brand(GoogleError, "configuration") {
...
* ```
*/
var PromptBlockedError = class PromptBlockedError extends ns.brand(GoogleError, "prompt-blocked") {
	...
	*/
	safetyRatings;
	constructor(params) {
		const message = params.message ?? `Prompt was blocked: ${params.blockReason}.${params.safetyRatings && params.safetyRatings.length > 0 ? ` Safety ratings: ${JSON.stringify(params.safetyRatings)}` : ""}`;
		super(message);
	...
	* ```
	*/
	static fromPromptFeedback(promptFeedback) {
...
* ```
*/
var AuthError = class AuthError extends ns.brand(GoogleError, "auth") {
	name = "AuthError";
	/**
	...
	*/
	static async fromResponse(response) {
		const errorBody = await require_misc.iife(async () => {
			try {
		...
		const message = errorBody?.error_description ?? `Authentication failed with status code ${response.status}`;
		const headers = require_misc.iife(() => {
			const object = {};
...
* ```
*/
var RequestError = class RequestError extends ns.brand(GoogleError, "request") {
	name = "RequestError";
	/**
	...
	*/
	static async fromResponse(response) {
		...
		const message = errorBody?.error?.message ?? errorBody?.message ?? errorBody?.error ?? `Request failed with status code ${response.status}`;
		const headers = require_misc.iife(() => {
			const object = {};
			response.headers.forEach((value, name) => {
				object[name] = value;
...
* ```
*/
var NoCandidatesError = class extends ns.brand(GoogleError, "no-candidates") {
	name = "NoCandidatesError";
	constructor() {
		super("No candidates returned from API. This may indicate the prompt was blocked or an error occurred.");
	}
};
/**
...
* ```
*/
var InvalidToolError = class extends ns.brand(GoogleError, "invalid-tool") {
	...
	*/
	...
	constructor(tool, message) {
		const defaultMessage = `Received invalid tool: ${JSON.stringify(tool)}. Tools must be LangChain tools, OpenAI tools, or Gemini function declarations.`;
...
* ```
*/
var ToolCallNotFoundError = class extends ns.brand(GoogleError, "tool-call-not-found") {
	name = "ToolCallNotFoundError";
	/**
	* The tool call ID that could not be found in the conversation history.
	*/
	toolCallId;
	constructor(toolCallId, message) {
...
* ```
*/
var MalformedOutputError = class extends ns.brand(GoogleError, "malformed-output") {
...
* ```
*/
var InvalidInputError = class extends ns.brand(GoogleError, "invalid-input") {
...
exports.InvalidToolError = InvalidToolError;
exports.MalformedOutputError = MalformedOutputError;
exports.NoCandidatesError = NoCandidatesError;
exports.PromptBlockedError = PromptBlockedError;
exports.RequestError = RequestError;
exports.ToolCallNotFoundError = ToolCallNotFoundError;
