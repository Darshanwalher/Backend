...
*/
function subtractTokenDetails(current, previous) {
	...
	for (const key of Object.keys(current)) {
		...
		if (cur !== void 0) result[key] = cur - (prev ?? 0);
	}
	return result;
}
/**
...
*/
function subtractUsageMetadata(current, previous) {
	if (!previous) return current;
	return {
		...
		input_token_details: subtractTokenDetails(current.input_token_details, previous.input_token_details),
		output_token_details: subtractTokenDetails(current.output_token_details, previous.output_token_details)
	};
}

//#endregion
exports.subtractUsageMetadata = subtractUsageMetadata;
