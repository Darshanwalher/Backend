...
*/
var NodeApiClient = class extends require_index.ApiClient {
	hasApiKey() {
		return typeof this.apiKey === "string" && this.apiKey !== "";
	}
	async getProjectId() {
		if (typeof this.credentials !== "undefined") return this.credentials.project_id;
		else if (typeof this.googleAuth !== "undefined") return this.googleAuth.getProjectId();
		else return super.getProjectId();
	}
	/**
	...
	*/
	constructor(params = {}) {
		super();
		this.params = params;
		this.apiKey = params.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("GOOGLE_API_KEY");
		this.credentials = require_misc.iife(() => {
			if (params.credentials) return require_gcp_auth.normalizeGCPCredentials(params.credentials);
			const credentialsVar = (0, _langchain_core_utils_env.getEnvironmentVariable)("GOOGLE_CLOUD_CREDENTIALS");
			if (credentialsVar) return require_gcp_auth.normalizeGCPCredentials(credentialsVar);
		});
		this.googleAuth = require_misc.iife(() => {
			if (params.googleAuthOptions) return new google_auth_library.GoogleAuth(params.googleAuthOptions);
			if (!this.apiKey && !this.credentials) return new google_auth_library.GoogleAuth();
		});
	}
	/**
	...
	*/
	async fetch(request) {
		if (this.apiKey) request.headers.set(require_const.GOOGLE_API_KEY_HEADER, this.apiKey);
		else if (this.credentials) request.headers.set(require_const.GCP_API_KEY_HEADER, `Bearer ${require_gcp_auth.getGCPCredentialsAccessToken(this.credentials)}`);
		else if (this.googleAuth) (await this.googleAuth.getRequestHeaders(request.url)).forEach((value, key) => {
...
*/
function ensureAuthScopes(authOptions, requiredScopes) {
	if (!authOptions) return { scopes: requiredScopes };
	else if (!authOptions.scopes) return {
		...authOptions,
		scopes: requiredScopes
	};
	else {
		const existingScopes = Array.isArray(authOptions.scopes) ? authOptions.scopes : [authOptions.scopes];
		if (requiredScopes.filter((scope) => !existingScopes.includes(scope)).length > 0) throw new require_errors.ConfigurationError(`Invalid auth scopes. Scopes must include ${requiredScopes.join(", ")}`);
	}
	return authOptions;
}
...
exports.ensureAuthScopes = ensureAuthScopes;
