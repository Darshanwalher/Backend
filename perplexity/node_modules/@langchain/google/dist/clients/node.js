...
import { getGCPCredentialsAccessToken, normalizeGCPCredentials } from "../utils/gcp-auth.js";
...
*/
var NodeApiClient = class extends ApiClient {
	hasApiKey() {
	...
	*/
	constructor(params = {}) {
		...
		this.credentials = iife(() => {
			if (params.credentials) return normalizeGCPCredentials(params.credentials);
			const credentialsVar = getEnvironmentVariable("GOOGLE_CLOUD_CREDENTIALS");
			if (credentialsVar) return normalizeGCPCredentials(credentialsVar);
		});
		this.googleAuth = iife(() => {
			if (params.googleAuthOptions) return new GoogleAuth(params.googleAuthOptions);
			if (!this.apiKey && !this.credentials) return new GoogleAuth();
		});
	}
	/**
	...
	*/
	async fetch(request) {
		if (this.apiKey) request.headers.set(GOOGLE_API_KEY_HEADER, this.apiKey);
		else if (this.credentials) request.headers.set(GCP_API_KEY_HEADER, `Bearer ${getGCPCredentialsAccessToken(this.credentials)}`);
...
function ensureAuthScopes(authOptions, requiredScopes) {
	if (!authOptions) return { scopes: requiredScopes };
		...
		if (requiredScopes.filter((scope) => !existingScopes.includes(scope)).length > 0) throw new ConfigurationError(`Invalid auth scopes. Scopes must include ${requiredScopes.join(", ")}`);
	}
	return authOptions;
}

//#endregion
export { NodeApiClient, ensureAuthScopes };
