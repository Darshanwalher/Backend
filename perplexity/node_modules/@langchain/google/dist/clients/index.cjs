...
*/
var WebApiClient = class extends ApiClient {
	hasApiKey() {
	...
	*/
	async fetch(request) {
		if (!this.apiKey && !this.credentials) throw new require_errors.AuthError({ message: "No authentication method available. Please provide either an apiKey or credentials." });
		if (this.apiKey) request.headers.set(require_const.GOOGLE_API_KEY_HEADER, this.apiKey);
		if (this.credentials) request.headers.set(require_const.GCP_API_KEY_HEADER, `Bearer ${await require_gcp_auth.getGCPCredentialsAccessToken(this.credentials)}`);
		return fetch(request);
	}
};
