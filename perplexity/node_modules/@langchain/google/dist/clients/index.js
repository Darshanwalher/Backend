...
var WebApiClient = class extends ApiClient {
	...
	*/
	async fetch(request) {
		if (!this.apiKey && !this.credentials) throw new AuthError({ message: "No authentication method available. Please provide either an apiKey or credentials." });
		if (this.apiKey) request.headers.set(GOOGLE_API_KEY_HEADER, this.apiKey);
		if (this.credentials) request.headers.set(GCP_API_KEY_HEADER, `Bearer ${await getGCPCredentialsAccessToken(this.credentials)}`);
		return fetch(request);
	}
};

//#endregion
export { ApiClient, WebApiClient };
