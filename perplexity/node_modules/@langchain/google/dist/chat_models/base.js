import { ConfigurationError, MalformedOutputError, NoCandidatesError, PromptBlockedError, RequestError } from "../utils/errors.js";
import PROFILES from "./profiles.js";
import { SafeJsonEventParserStream } from "../utils/stream.js";
import { convertAIMessageToText, convertGeminiCandidateToAIMessage, convertGeminiGenerateContentResponseToUsageMetadata, convertGeminiPartsToToolCalls, convertMessagesToGeminiContents, convertMessagesToGeminiSystemInstruction } from "../converters/messages.js";
import { convertToolChoiceToGeminiConfig, convertToolsToGeminiTools, schemaToGeminiParameters } from "../converters/tools.js";
import { convertFieldsToSpeechConfig, convertFieldsToThinkingConfig, convertParamsToPlatformType } from "../converters/params.js";
...
var BaseChatGoogle = class extends BaseChatModel {
	model;
	streaming;
	disableStreaming;
	streamUsage = true;
	...
	constructor(params) {
		...
		if (!params.apiClient) throw new ConfigurationError("BaseChatGoogle requires an apiClient. This should be provided automatically by ChatGoogle constructors. If you're extending BaseChatGoogle directly, please provide an apiClient instance.");
	...
	*/
	...
	invocationParams(options) {
		...
		const tools = fields.tools ? convertToolsToGeminiTools(fields.tools) : void 0;
		const toolConfig = convertToolChoiceToGeminiConfig(options.tool_choice, !!(tools && tools.length > 0));
		let responseJsonSchema;
		let responseMimeType;
		if (fields.responseSchema) {
			if (isInteropZodSchema(fields.responseSchema) || isSerializableSchema(fields.responseSchema)) responseJsonSchema = toJsonSchema(fields.responseSchema);
	...
	async _generate(messages, options, runManager) {
		...
		if (data.promptFeedback?.blockReason) throw PromptBlockedError.fromPromptFeedback(data.promptFeedback);
		if (!data.candidates || data.candidates.length === 0) throw new NoCandidatesError();
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		if (response.body) {
			let previousUsage;
			const reader = response.body.pipeThrough(new TextDecoderStream()).pipeThrough(new EventSourceParserStream()).pipeThrough(new SafeJsonEventParserStream()).pipeThrough(new TransformStream({ transform(chunk, controller) {
				runManager?.handleCustomEvent(`google-chunk-${moduleName}`, { chunk });
				if (chunk === null) {
					controller.enqueue(new ChatGenerationChunk({
						text: "",
				...
				if (parts.length > 0 || candidate.finishReason) {
					...
					if (chunk?.usageMetadata && streamUsage) {
						const cumulative = convertGeminiGenerateContentResponseToUsageMetadata(chunk);
						messageChunkParams.usage_metadata = subtractUsageMetadata(cumulative, previousUsage);
	...
	*/
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonSchema") {
			...
			outputParser = RunnableLambda.from(async (input) => {
				if (!AIMessage.isInstance(input) && !AIMessageChunk.isInstance(input)) throw new MalformedOutputError({ message: "Input is not an AIMessage or AIMessageChunk." });
				if (!input.text) throw new MalformedOutputError({ message: "No content found in response. Cannot parse structured output." });
				return await createContentParser(schema).parse(input.text);
			});
		...
		} else throw new ConfigurationError(`Unrecognized structured output method '${method}'. Expected 'functionCalling' or 'jsonSchema'`);
		return assembleStructuredOutputPipeline(llm, outputParser, includeRaw, includeRaw ? "ChatGoogleStructuredOutputRunnable" : "ChatGoogleStructuredOutput");
	}
};
...
//#endregion
export { BaseChatGoogle, getGoogleChatModelParams, getPlatformType };
